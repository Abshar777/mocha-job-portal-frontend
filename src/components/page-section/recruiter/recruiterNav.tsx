...
import { RecruiterMobileNavLinks, RecruiterNavLinks } from "@/constants/recruiter"
  ...
  return (
    <div className="md:flex hidden items-center gap-6">
        ...
        <Link key={link.href} className="group relative" href={link.href}>
          ...
          <div
            className={cn(
              ...
              link.href !== pathname ? "scale-x-0 " : "scale-x-100",
              link.href !== pathname && "group-hover:scale-x-100 group-hover:origin-left",
            )}
          ></div>
        </Link>
      ))}
      ...
      <div className="flex items-center justify-between overflow-hidden bg-white/5 rounded-full">
        <Button size="md" isIconOnly className="bg-transparent px-2 text-lg text-white">
          <HiOutlineMenuAlt1 />
        </Button>
        <Button size="md" isIconOnly className="rounded-full px-2 bg-white/10 shadow-md text-sm text-white">
          <FaUserAlt />
        </Button>
      </div>
    </div>
...
const MobileNavSheetContent = ({ closeDrawer }: { closeDrawer: Function }) => {
  ...
  return (
    <Drawer.Portal>
      <Drawer.Overlay className="fixed z-50 inset-0 bg-black/40 backdrop-blur-sm" />
      <Drawer.Content
        ...
        <div className="bg-background h-full w-full grow px-3 py-1 flex flex-col rounded-[16px]">
          <Drawer.Title className="font-medium px-0 -translate-y-3 border-b border-dashed justify-between flex items-center mb-2 text-white">
            <div className="top-0 -translate-x-3 scale-[.8] left-0">
              <Image src={Logo || "/placeholder.svg"} alt="logo" className="object-contain h-full" />
            </div>
            <Button
              onPress={() => closeDrawer()}
              size="sm"
          ...
          <Drawer.Description className="mb-2 flex flex-col gap-4">
            {RecruiterMobileNavLinks.map((e) => (
              <Link
                className={cn("w-full flex items-center justify-between", pathname == e.href && "text-primary")}
                key={e.href}
                href={e.href}
              >
                <span className="">{e.label}</span>
                <IoIosArrowDroprightCircle className={cn("text-primary text-lg", pathname != e.href && "opacity-50")} />
              </Link>
            ))}
          </Drawer.Description>
        </div>
...
const RecruiterNav = () => {
  ...
  return (
    <div className="flex bg-foreground p-4 w-full h-16 items-center justify-between">
      <Drawer.Root shouldScaleBackground open={isOpen} onOpenChange={setIsOpen} direction="left">
